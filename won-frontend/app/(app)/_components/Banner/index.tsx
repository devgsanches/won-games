...
export type BannerProps = GetHomeQuery['banners'][number]
    ...
    >
      <div className="md:hidden h-full">
        <div
          ...
          style={{
            backgroundImage: `url('http://localhost:1337${image.url ?? 'https://i0.wp.com/espaferro.com.br/wp-content/uploads/2024/06/placeholder-103.png?fit=1200%2C800&ssl=1&w=640'}')`
          }}
        />
        ...
        >
          {ribbon?.ribbonText && (
            <div className="relative w-full h-full overflow-visible">
              <div className="absolute w-full h-full overflow-visible">
                <Ribbon color={ribbon.color ?? 'primary'} size={ribbon?.size ?? 'large'}>{ribbon.ribbonText}</Ribbon>
              </div>
            </div>
          )}
          <div className="h-55.5 absolute bottom-0 bg-black/70 w-full pt-8 px-11 flex flex-col gap-6 rounded-b">
            <div>
              ...
              <p className="text-xlarge first-letter:uppercase">
                jogue a nova temporada de {""}
                <span className="text-primary font-bold">{title}</span>
              </p>
            </div>
